...
class Mouse:
    def __init__(self, starting_position, plateau, method='random'):
        self.case_number = starting_position
        if method == 'random':
            self.case_number = random.choice(range(plateau.n_cols * plateau.n_rows))
        while not plateau.cases[self.case_number].is_allowed_to_mouse or plateau.cases[self.case_number].has_cat:
            self.case_number = random.choice(range(plateau.n_cols * plateau.n_rows))
        self.plateau = plateau
        self.plateau.cases[self.case_number].has_mouse = True
    ...
    def move_down(self):
        ...
        self.plateau.cases[self.case_number].has_mouse = False
        if self.case_number + self.plateau.n_cols < self.plateau.n_cols * self.plateau.n_rows:
            ...
            if not self.plateau.cases[self.case_number].is_allowed_to_mouse:
...
class Mouses:
    def __init__(self, plateau, number_of_mouses):
        self.mouses = []
        self.plateau = plateau
        for _ in range(number_of_mouses):
            self.mouses.append(Mouse(0, self.plateau))
            
    def is_done(self):
        return len(self.mouses) == 0
    
    def kill_mouse(self, position):
        self.mouses.pop(position)
