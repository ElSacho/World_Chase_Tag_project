...
PARTICULAR_NAME ='nothing_5x5_simple_reward'
VISION = 4
N_ROWS = 5
N_COLS = 5
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if frame_idx < EPSILON_DECAY_LAST_FRAME:
            ...
            cat_loss_t.backward()
            cat_optimizer.step()
        # print(f'after : {mouse_reward}')
        #train the mouse
        if frame_idx > EPSILON_DECAY_LAST_FRAME:
            epsilon = max(EPSILON_FINAL, EPSILON_START -
                      (frame_idx - EPSILON_DECAY_LAST_FRAME) / EPSILON_DECAY_LAST_FRAME)
            try :
                if mouse_reward != -1:
                    # print("reward not -1")
            ...
            if mouse_reward is not None or cat_reward is not None:
                # mouse_reward = mouse_agent.total_reward
            ...
            if frame_idx > 600_000:
                torch.save(mouse_net.state_dict(), mouse_folder_name +"/"+ args.env +
                            "-best_10000.dat" % mouse_m_reward)
                torch.save(cat_net.state_dict(), cat_folder_name +"/"+ args.env +
                            "-best_10000.dat" % cat_m_reward)
                break
            mouse_reward = None
